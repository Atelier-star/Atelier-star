...
// Энэ хувилбарт "ТАНЫ ЗАХИАЛГА" summary хэсгийг checkout-оос авч,
// хэрэглэгчийн мэдээлэл → хүргэлт → төлбөр → захиалга гэсэн урсгал ашиглана.
...
if (placeOrderButton) {
    ...
    placeOrderButton.addEventListener("click", async function(event) {
        ...
        // Давхар даралтаас хамгаална
        if (placeOrderButton.disabled) return;
        placeOrderButton.disabled = true;
        placeOrderButton.textContent = "БҮРТГЭЖ БАЙНА...";
        ...
        if (!name) {
            ...
            placeOrderButton.textContent = "ЗАХИАЛГА ӨГӨХ";
            return;
        }
        ...
        // Хүргэлтийн үнэ
        const deliveryFee =
            deliveryZone && deliveryZone.value === "local"
                ? 10000
                : 5000;
        ...
        const paymentNames = {
            cash: "Хүргэлтийн үед бэлнээр төлөх",
            qpay: "QPay",
            bank: "Банкны дансаар төлөх",
            card: "Карт"
        };

        const paymentMethod =
            paymentNames[selectedPayment ? selectedPayment.value : "cash"];

        const finalTotal =
            productTotal + deliveryFee;

        const orderNumber =
            "ATL-" + Date.now().toString().slice(-6);

        // Нэвтэрсэн хэрэглэгчийг авна
        ...
        const { error: orderError } =
            await supabaseClient
                ...
                .insert({
                    ...
                    product_total: newOrder.productTotal,
        ...
        if (orderError) {
            ...
            alert("Захиалгыг хадгалахад алдаа гарлаа.");
            placeOrderButton.disabled = false;
            placeOrderButton.textContent = "ЗАХИАЛГА ӨГӨХ";
            return;
        }

        // Хуучин localStorage захиалгын түүхийг мөн хадгална
        let orders = JSON.parse(
            localStorage.getItem("atelierOrders")
        ) || [];
        ...
        // Confirmation мэдээлэл
        document.getElementById("successOrderNumber").textContent =
            orderNumber;

        document.getElementById("successProductTotal").textContent =
            "₮" + productTotal.toLocaleString();

        document.getElementById("successDeliveryFee").textContent =
            "₮" + deliveryFee.toLocaleString();

        document.getElementById("successDeliveryName").textContent =
            deliveryName;
        ...
        // Checkout-ийг БҮРЭН нууж, зөвхөн confirmation үлдээнэ
        ...
        // Дахин захиалга өгөхөөс хамгаална
        cart = [];
        saveCart();
